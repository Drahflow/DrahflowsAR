...
public class ScaleCalibrationActivity implements ArActivity {
	...
	public ScaleCalibrationActivity(GlobalState global_) {
		...
		menu.add(new Menu.Element() {
			...
			public void onClick() {
				global.mapScale -= 0.1f;
				JNI.SVO_setMapScale(global.mapScale);
			}
		});
		menu.add(new Menu.Element() {
			{ x = 0.05f; y = 0.05f; s = 0.01f; }
		...
		menu.add(new Menu.Element() {
			{ x = -0.05f; y = 0.00f; s = 0.01f; }

			public void draw(Menu.Renderer r) {
				draw(r, Menu.RED);
			}
			public void onClick() {
				global.eyeDistance -= 0.01f;
			}
		});
		menu.add(new Menu.Element() {
			{ x = 0.05f; y = 0.00f; s = 0.01f; }
		...
		menu.add(new Menu.Element() {
			{ x = -0.05f; y = -0.02f; s = 0.01f; }

			public void draw(Menu.Renderer r) {
				draw(r, Menu.RED);
			}
			public void onClick() {
				global.eyeDistance -= 0.001f;
			}
		});
		menu.add(new Menu.Element() {
			{ x = 0.05f; y = -0.02f; s = 0.01f; }
		...
		menu.add(new Menu.Element() {
			{ x = -0.05f; y = -0.07f; s = 0.01f; }

			public void draw(Menu.Renderer r) {
				draw(r, Menu.RED);
			}
			public void onClick() {
				global.eyeLookDistance -= 0.001f;
			}
		});
		menu.add(new Menu.Element() {
			{ x = 0.05f; y = -0.07f; s = 0.01f; }
	...
	public class DevelopmentRenderer implements GLSurfaceView.Renderer {
		...
		public void onDrawFrame(GL10 glUnused) {
			...
			GLES20.glViewport(0, 0, width / 2, height);
			Matrix.setLookAtM(lookAtMatrix, 0, eyeX - global.cameraDistance - global.eyeDistance, eyeY, eyeZ, lookX - global.eyeLookDistance, lookY, lookZ, upX, upY, upZ);
			drawEyeView(pose);
			Utils.noGlError();

			GLES20.glViewport(width / 2, 0, width / 2, height);
			Matrix.setLookAtM(lookAtMatrix, 0, eyeX - global.cameraDistance, eyeY, eyeZ, lookX, lookY, lookZ, upX, upY, upZ);
			drawEyeView(pose);
			Utils.noGlError();
